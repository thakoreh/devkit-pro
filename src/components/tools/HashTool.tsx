...
export default function HashTool() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="text-sm font-medium mb-2 block" style={{ color: 'var(--text-secondary)' }}>Input String</label>
        <textarea className="input-area" value={input} onChange={e => setInput(e.target.value)} placeholder="Enter text to hash..." rows={4} />
      </div>
      {hashes && (
          ...
          {Object.entries(hashes).map(([algo, hash]) => (
            <div key={algo} className="tool-card p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>{algo}</span>
                <button onClick={() => navigator.clipboard.writeText(hash)} className="btn-copy text-xs">📋 Copy</button>
              </div>
              <code className="text-xs break-all" style={{ color: 'var(--accent)' }}>{hash}</code>
            </div>
          ))}
        </div>
      )}
